...
pub static SETUP: Setup = Setup {
    camera_device: 4,                      // Default to first camera
...
fn main() -> Result<()> {
    ...
    if !test {
        ...
        let (vcam_tx, vcam_rx) = std::sync::mpsc::channel::<Vec<u8>>();
        ...
        join_handles.push(
            thread::Builder::new()
                ...
                .spawn(move || -> Result<()> { viewer::start_raylib_viewer(raylib_rx, vcam_tx) })?,
        );

        join_handles.push(thread::Builder::new().name("virtual_cam".into()).spawn(move || {
            virtual_camera::virtual_cam(vcam_rx)?;
            Ok(())
        })?);
